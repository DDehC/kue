...
/* Admin scope: no student management */
const VISIBLE_ROLES = ["staff", "admin"];
...
export default function AdminUsersPage() {
  ...
  const [role, setRole] = useState("all"); // "all" | "staff" | "admin"
  ...
  async function load() {
    ...
    setError("");
    try {
      // Ask backend for the selected role if not "all"; otherwise let it return everything
  ...
  useEffect(() => { load(); }, []);
  useEffect(() => { load(); }, [q, role, onlyActive]);

  // Enforce visibility rule on the client too (in case backend still returns students)
...
/* Modal: restrict role options to staff/admin; default to staff */
